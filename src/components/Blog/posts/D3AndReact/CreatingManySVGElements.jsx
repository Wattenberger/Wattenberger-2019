  ...
  return (
    <div className="CreatingManySVGElements">
      <p>
          ...
          <Aside>
          If you're unfamiliar with d3.s data binding concept, I'd recommend reading up on it <Link to="https://observablehq.com/@d3/selection-join">on Observable</Link> or getting a full rundown with <Link to="https://newline.co/fullstack-d3">Fullstack Data Visualization and D3</Link>.
          </Aside>
      </p>
      ...
      <Expandy trigger="How are we generating this data?">
        <p>
          We've created a <P>generateDataset()</P> function that outputs an array of 10 arrays.
        </p>
              ...
              <div style={{
                ...
                {dataset.map(([x, y], i) => (
                  <div key={i} style={{
                    ...
                    [
                      <span style={{ display: "inline-block", width: "4em", textAlign: "right" }}>{ format(".2f")(x) }</span>,
                      <span style={{ display: "inline-block", width: "4em", textAlign: "right" }}>{ format(".2f")(y) }</span>
                    ],
                  </div>
                ))}
                <div>]</div>
        ...
        <div className="D3AndReact__side-by-side">
            <div className="D3AndReact__side-by-side__section">
                ...
                ]}>{ generateDatasetCode }</Code>
            </div>
        </div>
      </Expandy>

      <p>
        What would it look like if we drew a <P>{`<circle>`}</P> at each of these locations? Starting with the naive d3 code:
      </p>
          ...
          example={setHighlightedMarker => (
            <>
              ...
              <List className="D3AndReact__marked-list" items={[
                ...
                <Aside>
                  Note that <P>{`<svg>`}</P>s are, by default, 300px x 150px - in this case, we're using the <Link kto="https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/viewBox"><P>viewBox</P></Link> element to re-define our working grid, just so we can use friendlier numbers.
                </Aside>
      ...
      <p>
        Okay, we're back to our original issues: our code is a bit <b>imperative</b>, <b>verbose</b>, and <b>hacky</b>. What would this look like using React to render our <P>{`<circle>`}</P>s?
      </p>
      ...
      <CodeAndExample
        ...
        <p>
          We all know that d3 is great at keeping track of what elements are new and animating elements in and out. And if you don't you should <Link to="https://newline.co/fullstack-d3">read a book</Link>.
        </p>
        ...
        <CodeAndExample
          ...
          example={setHighlightedMarker => (
            <>
              ...
              <p>
                <b>Don't feel the need to run through all of it</b> -- the main gist is that we have 6 <P>{`<circle>`}</P>s, and every two seconds, we randomly choose some of them to show up.
              </p>
        ...
        <CodeAndExample
          ...
          example={setHighlightedMarker => (
            <>
              ...
              <p>
                Animating elements <i>out</i> is not very straightforward in React, so let's keep all of the <P>{`<circle>`}</P>s rendered, and give them an <P>opacity</P> if they're not in the currently shown circles.
              </p>
        ...
        <p>
          While this code isn't <i>shorter</i> than the d3 code, it is a lot easier to read.
        </p>
    </div>
  )
}
...
const TransitionsWithReactCode =
`const AnimatedCircles = () => {
  const [visibleCircles, setVisibleCircles] = useState(
    generateCircles()
  )
